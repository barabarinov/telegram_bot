...
def other(update: Update, context: CustomContext) -> None:
    """Display a message with a button."""
    update.message.reply_html(
        'This button was clicked <i>0</i> times.',
        reply_markup=InlineKeyboardMarkup.from_button(
            InlineKeyboardButton(text='Click me!', callback_data='other')
        ),
    )
...
def main() -> None:
    ...
    load_dotenv()
    updater = Updater(token=os.getenv('TOKEN'), context_types=context_types)
